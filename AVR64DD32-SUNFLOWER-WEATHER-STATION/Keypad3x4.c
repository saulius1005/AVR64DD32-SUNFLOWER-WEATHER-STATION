 ...
 */
uint8_t scan_keypad() {
    uint8_t row_pins[4] = {PIN0_bm, PIN1_bm, PIN2_bm, PIN3_bm}; ///< Row pins
    uint8_t col_pins[3] = {PIN4_bm, PIN5_bm, PIN6_bm}; ///< Column pins

    for (uint8_t row = 0; row < 4; row++) {
        PORTF.OUTCLR = row_pins[row]; // Set the current row pin low
        for (uint8_t col = 0; col < 3; col++) {
            if (!(PORTF.IN & col_pins[col])) {
                PORTF.OUTSET = row_pins[row]; // Reset the row pin
