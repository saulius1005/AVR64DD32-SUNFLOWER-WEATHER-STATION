 ...
 * SSD1306.c
 *
 * Created: 2025-01-22 21:31:32
 *  Author: Saulius
 */ 

#include "Settings.h"
#include "SSD1306Var.h"
#include "font.h"
...
void screen_command(uint8_t cmd) {
    WriteToReg(SSD1306_ADD, 0x00, cmd);  ///< Send command to ST7567S display
}
...
void screen_data(uint8_t data) {
    WriteToReg(SSD1306_ADD, 0x40, data);  ///< Send data to ST7567S display
}
...
void screen_init() { 
    ...
    for (uint8_t i = 0; i < SSD1306_CMD_COUNT; i++) {
        TransmitByte(ssd1306Commands[i]);  ///< Send initialization command
    }
}
